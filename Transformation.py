...
def segmenting_red_green(numpy_array):
    ...
    # gaussian blur calculate the mean of neighbor: produce smooth and
    # soft edges
    gaussian_img = pcv.gaussian_blur(
        img=otsu, ksize=(5, 5), sigma_x=0, sigma_y=None
    )
    leaf_mask = pcv.fill_holes(bin_img=otsu)
    masked = pcv.apply_mask(
            img=numpy_array, mask=leaf_mask, mask_color="white"
        )
...
def transform_image(image_path):
    ...
    combine_gradients, combine_edges = combine_edge_detection(
    ...
    b_channel, median_img, otsu, gaussian_img, leaf_mask_b, masked = (
        segmenting_blue_yellow(numpy_image)
    )
    s_channel, median_img, otsu, gaussian_img, leaf_mask_s, masked = (
        segmenting_saturation(numpy_image)
    )
    ...
    masked = pcv.apply_mask(
        img=numpy_image, mask=leaf_mask_c, mask_color="white"
    )

    pcv.params.line_thickness = 2
    size_analysis_image = pcv.analyze.size(
        img=numpy_image, labeled_mask=leaf_mask_z
    )
...
def save_transformed(
    source, image_path, images_with_titles: list[tuple], destination
):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="testing the Transformation of open CV class"
    )
    parser.add_argument(
        "-src", help="the directory to parse from", required=False
    )
    parser.add_argument(
        "-dst", help="the directory to write to", required=False
    )
    parser.add_argument(
        "filename",
        help="a single filename to transform",
        nargs="?",
        default=None,
    )
    ...
    if args.filename and not (args.src or args.dst):
        ...
        print(
            f"Reading from source: {args.src} \
               and write to destination: {args.dst}"
        )
        transform_dataset(Path(args.src), Path(args.dst))
    else:
        parser.error(
            "You must provide either a filename \
               or both -src and -dst options."
        )
        parser.print_help()
